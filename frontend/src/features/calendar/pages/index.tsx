...
const CalendarLeave: React.FC = () => {
  ...
  const [dayDurations, setDayDurations] = useState<Record<string, "full" | "half" | "afternoon">>({});
...
//   // Only open modal if leaveType is NOT selected from side panel
//   // Assuming "leaveType" has a default value if not selected, you can track a separate flag
//   const isLeaveTypeSelectedInPanel = leaveType !== ""; // or another flag if needed

//   if (!isLeaveTypeSelectedInPanel) {
//     setClickedDate(date);
//     setModalOpen(true);
//   } else {
//     // If leave type is selected in side panel, just add date to selectedDates
...
const handleDayClick = (date: Date) => {
  ...
  if (!isLeaveTypeSelectedInPanel) {
    ...
    if (!selectedDates.some(d => d.toISOString().split("T")[0] === key)) {
      ...
      setSelectedDates(selectedDates.filter(d => d.toISOString().split("T")[0] !== key));
  ...
  const handleDurationChange = (date: Date, value: "full" | "half" | "afternoon") => {
    const key = date.toISOString().split("T")[0];
  ...
  const handleApplyLeaveFromSidePanel = () => {
    selectedDates.forEach(d => applyLeave(d.toISOString().split("T")[0], leaveType));
    setSelectedDates([]);
    setDayDurations({});
  };
